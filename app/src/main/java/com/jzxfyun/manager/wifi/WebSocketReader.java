...
class WebSocketReader extends Thread {
    ...
    public void quit() {
        mState = STATE_CLOSED;
        CommonUtils.log().d(getName(),"Quit");
    }
    ...
    private Map<String, String> parseHttpHeaders(String[] httpResponse) throws UnsupportedEncodingException {
        ...
        for (String line : httpResponse) {
            if (line.length() > 0) {
                ...
                if (h.length == 2) {
                    headers.put(h[0], h[1]);
                    CommonUtils.log().d(getName(), String.format("'%s'='%s'", h[0], h[1]));
                }
            }
        }
    ...
    private Pair<Integer, String> parseHttpStatus(String statusLine) throws UnsupportedEncodingException {
        ...
        CommonUtils.log().d(getName(), String.format("Status: %d (%s)", statusCode, statusMessage));
    ...
    public void run() {
        ...
        CommonUtils.log().d(getName(),"Running");
        ...
        try {
            do {
                ...
                if (len > 0) {
                    ...
                    CommonUtils.log().d(getName(),"run() : ConnectionLost");
            ...
            CommonUtils.log().d(getName(),"run() : WebSocketException (" + e.toString() + ")");
            ...
            CommonUtils.log().d(getName(),"run() : Exception (" + e.toString() + ")");
        ...
        CommonUtils.log().d(getName(),"Ended");
    }
}
