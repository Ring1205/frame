...
import com.jzxfyun.common.base.fragment.SupportHelper;
...
import db.zxycloud.map.poi.MyPoiOverlay;

public class MapViewActivity extends BaseActivity implements LocationSource, PoiSearch.OnPoiSearchListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        this.listener = new OnMapLocationListener() {
            @Override
            public void onRegeocodeSearched(RegeocodeResult regeocodeResult, int i) {
                latLng = regeocodeResult.getRegeocodeQuery().getPoint();
                regeocodeAddress = regeocodeResult.getRegeocodeAddress();
                cityCode = regeocodeResult.getRegeocodeAddress().getCityCode();
                ((EditText) findViewById(R.id.et_area)).setText(regeocodeAddress.getFormatAddress());
            }
        };
        initSearch();
        setOnClickView(R.id.btn_save);
    }
    ...
    protected void onViewClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_search:
                ...
                break;
            case R.id.btn_save:
                if (((EditText) findViewById(R.id.et_address)).getText().toString().isEmpty()) {
                    CommonUtils.toast(this, R.string.toast_detailed_address);
                } else {
                    Intent data = new Intent();
                    Bundle bundle = new Bundle();
                    bundle.putString("name", getIntent().getStringExtra("name"));
                    bundle.putString("password", getIntent().getStringExtra("password"));
                    bundle.putString("sheng", regeocodeAddress.getProvince());
                    bundle.putString("shi", regeocodeAddress.getCity());
                    bundle.putString("qu", regeocodeAddress.getDistrict());
                    bundle.putString("address", ((EditText) findViewById(R.id.et_address)).getText().toString());
                    bundle.putDouble("lat", latLng.getLatitude());
                    bundle.putDouble("lon", latLng.getLongitude());
                    data.putExtra("map", bundle);
...
//        mUiSettings.setMyLocationButtonEnabled(false);
        ...
        setOnClickView(R.id.tv_location);
        return aMap;
    }
    ...
    public void initSearch() {
        ...
        recyclerCity = findViewById(R.id.recycler);
        recyclerCity.setLayoutManager(new LinearLayoutManager(this));
        adapter = new MyBaseAdapter(this, R.layout.map_view_item, new MyBaseAdapter.OnBindViewHolderListener() {
            @Override
            public void onBindViewHolder(final int position, View view, RecyclerViewHolder holder) {
                holder.setText(R.id.tv_address, poiItems.get(position).getTitle());
                holder.setText(R.id.tv_address_info, poiItems.get(position).getSnippet());
                holder.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        latLng = poiItems.get(position).getLatLonPoint();
                        ((EditText) findViewById(R.id.et_area)).setText(poiItems.get(position).getSnippet());
                        ((EditText) findViewById(R.id.et_address)).setText(poiItems.get(position).getTitle());
                        aMap. moveCamera(CameraUpdateFactory.changeLatLng(new LatLng(latLng.getLatitude(), latLng.getLongitude())));
                        SupportHelper.hideSoftInput(getWindow().getDecorView());
                        recyclerCity.setVisibility(View.GONE);
                        inputEdittext.setText(poiItems.get(position).getTitle());
                        inputEdittext.setSelection(poiItems.get(position).getTitle().length());
                    }
                });
            }
        });
        ...
        setOnClickView(R.id.btn_search);
    }
...
//                getAddressByLatlng(getMapCenterPoint());
    ...
    private void doSearchQuery(String keyWord) {
        query = new PoiSearch.Query(keyWord, "", cityCode != null ? cityCode : "");// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
        ...
        // 周边搜索
//        poiSearch.setBound(new PoiSearch.SearchBound(latLng, 50000, true));// 设置搜索区域为以latLng点为圆心，其周围5000米范围
...
//                aMap.addMarker(new MarkerOptions().position(new LatLng(lat, lng)));
    ...
    public void startJumpAnimation() {
        if (screenMarker != null) {
            ...
            point.y -= 5 * getResources().getDisplayMetrics().density + 0.5f;
